...
JB_INITRAMFS_SOURCE="/home/brett/kern/usr/initramfs/source.list"
RAMDISK="/home/brett/kern/ramdisk"
RAMDISK_OUT="/home/brett/kern/usr/initramfs/ramdisk-JB.cpio"
...
echo "Building JB Kernel..."
		cd ${KERNEL_DIR}
		make -j8 ARCH=arm CROSS_COMPILE=${TOOLCHAIN} CONFIG_INITRAMFS_SOURCE=${JB_INITRAMFS_SOURCE}
		cp arch/arm/boot/zImage ${OUTDIR}
		cd ${OUTDIR}
		echo "Creating Shift CWM kernel zip..."
		zip -r Shift-JELLY-5.0.zip ./ -x *.zip *.gitignore
